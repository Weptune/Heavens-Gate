#include "../src/evaluation/tensor_nnue.hpp"
...
int main(int argc, char* argv[]) {
    std::cout << "======================================================\n";
    std::cout << "  Heaven's Gate — Tensor-Decomposed NNUE Trainer      \n";
    std::cout << "  Tensor-Ring Feature Network Dataset Bootstrapper    \n";
    std::cout << "======================================================\n\n";
    ...
    std::cout << "[TensorNNUE] Simulating " << num_games << " Self-Play Games @ Depth " << depth << "...\n";
    ...
    for (int g = 0; g < num_games; g++) {
        ...
        while (moves_count < 80) {
            ...
            if (res.best_score > 400) {
                result_score = (board.side_to_move() == Color::White) ? 1 : -1;
                break;
            } else if (res.best_score < -400) {
                result_score = (board.side_to_move() == Color::White) ? -1 : 1;
                break;
            }
    ...
    std::cout << "\n[TensorNNUE] Dataset Generation Complete: " << dataset.size() << " position samples\n";
    std::cout << "  Outcomes: " << white_wins << " White Wins, " << black_wins << " Black Wins, " << draws << " Draws\n\n";

    TensorNNUE model(32);
    model.initialize_weights(42);

    std::cout << "[TensorNNUE] Training Tensor-Decomposed NNUE (Rank R=32, Epochs=" << epochs << ")...\n";
    ...
    std::string model_path = "heavensgate_tnnue.nnue";
    if (model.save_weights(model_path)) {
        std::cout << "\n[SUCCESS] Saved Tensor-Decomposed NNUE model to " << model_path << "\n";
    }
