...
# Phase lists (all in mover-relative / HG perspective)
op_hg_evals = []
op_sf_evals = []
op_hg_cpl = []
op_sf_cpl = []
...
for g_idx, g_text in enumerate(games, 1):
    ...
    for match in matches:
        ...
        # Mover-relative score (positive = good for mover)
        mover_score = clamped
        # Engine perspective (positive = good for HG)
...
print("=" * 85)
print("     HEAVEN'S GATE vs STOCKFISH 3400 — OPENING, MIDGAME & ENDGAME MASTERY")
print("=" * 85)
print(f"{'Game Phase':<24} {'Metric':<26} {'Heaven\'s Gate (Master)':<22} {'Stockfish 3400'}")
print("-" * 85)
