...
> **YouTube Episode Concept**: *"The Minimax Theorem: How Mathematics First Learned to Look Ahead"*
...
1. **Tree Graph Visualization (`game_tree.json`)**:
   - Render the complete minimax game tree generated at depth 3.
   - Color nodes by evaluation (blue for White advantage, red for Black advantage).
   - Animate the Minimax values bubbling up from leaf nodes to the root.
2. **Exponential Explosion Graph**:
   - Compare exponential \(O(b^d)\) curve against real time to show why classical search needs pruning.
...
> **Narrator**: 
> *"In our last video, our engine was a infant—it knew the rules of chess, but every move was a random dice roll.*
> *Today, we introduce a single mathematical idea proven by John von Neumann in 1928: The Minimax Theorem.*
> *By assuming your opponent is as smart as you are, you can mathematically prove which move guarantees the best possible outcome.*
> *Watch what happens when our engine looks 4 moves into the future for the first time..."*
