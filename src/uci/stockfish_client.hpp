...
public:
    ...
    SearchResult get_search_result_clock(const Board& board, int wtime_ms, int btime_ms, int winc_ms, int binc_ms) {
        SearchResult res;
        if (!is_running) {
            if (!init(2400)) return res;
        }
        ...
        std::string fen = FEN::to_string(board);
        send_cmd("position fen " + fen);
        send_cmd("go wtime " + std::to_string(wtime_ms) + " btime " + std::to_string(btime_ms) + " winc " + std::to_string(winc_ms) + " binc " + std::to_string(binc_ms));
